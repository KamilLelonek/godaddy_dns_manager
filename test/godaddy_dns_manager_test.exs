defmodule GodaddyDnsManagerTest do
  ...
  alias GodaddyDnsManager

  setup do
    {:ok, domain: Application.get_env(:godaddy_dns_manager, GodaddyDnsManager)[:domain]}
  end
  ...
  describe "create_record/1" do
    ...
    test "shoudl send PATCH request to GoDaddy with a specified data", %{url: url} do
      ...
      assert {:ok, %Tesla.Env{body: ^body}} = GodaddyDnsManager.create_record("RECORD")
    end
    ...
    test "should not send a PATCH request with invalid credentials", %{url: url} do
      ...
      assert {:ok, %Tesla.Env{status: 401, body: ^body}} = GodaddyDnsManager.create_record("RECORD")
    end
  end
  ...
  describe "cleanup_record/0" do
    ...
    test "should clear the existing TXT record", %{url: url} do
      ...
      assert {:ok, %Tesla.Env{body: ^body}} = GodaddyDnsManager.cleanup_record()
    end
  end
end
